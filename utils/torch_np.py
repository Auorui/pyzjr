...
def count_zero(thresh: NdarrayOrTensor) -> int:
    """Count the number of zero-value pixels in a thresholded image.

    Args:
        thresh: Input thresholded image (numpy array or torch tensor)

    Returns:
        int: Count of zero-value pixels

    Raises:
        TypeError: If input is not a numpy array or torch tensor
    """
    if isinstance(thresh, np.ndarray):
        return int(np.sum(thresh == 0))
    elif torch.is_tensor(thresh):
        return int(torch.sum(thresh == 0).item())
    raise TypeError("Input must be numpy array or torch tensor")


def count_white(thresh: NdarrayOrTensor) -> int:
    """Count the number of white (255-value) pixels in a thresholded image.

    Args:
        thresh: Input thresholded image (numpy array or torch tensor)

    Returns:
        int: Count of white pixels (value 255)
    ...
    if isinstance(thresh, np.ndarray):
        ...
        return int(torch.sum(thresh == 255).item())
    raise TypeError("Input must be numpy array or torch tensor")


def count_nonzero(thresh: NdarrayOrTensor) -> int:
    """Count the number of non-zero pixels in an image.

    Args:
        thresh: Input image (numpy array or torch tensor)

    Returns:
        int: Count of non-zero pixels
    ...
    if isinstance(thresh, np.ndarray):
        ...
        return int(torch.count_nonzero(thresh).item())
    raise TypeError("Input must be numpy array or torch tensor")
