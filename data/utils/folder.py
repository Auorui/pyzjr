...
def logdir(dir_path="logs", prefix="", suffix="", use_simple_format=True):
    """
    Logging generator
    :param dir_path: Default "logs"
    ...
    folder_keys_names = [prefix, time_str, suffix]
    folder_names = [folder for folder in folder_keys_names if folder]
    log_dir = os.path.join(dir_path, *folder_names)
...
def datatime_makedirs(dir_path, *paths, use_simple_format=True):
    """
    创建含有独特时间的路径
    """
    time_path = timestr(use_simple_format)
    outdir = os.path.join(dir_path, time_path, *paths)
    os.makedirs(outdir, exist_ok=True)
    return outdir

def loss_weights_dirs(dir_path='./logs', prefix=None, suffix=None, use_simple_format=True):
    """训练代码常用路径, 保存loss信息, 以及保存最佳模型"""
    time_dir = logdir(dir_path, prefix=prefix, suffix=suffix, use_simple_format=use_simple_format)
