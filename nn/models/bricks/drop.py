...
__all__ = ["DropPath", "Dropout", "DropBlock", "MultiSampleDropout", "DropConnect", "Standout", "GaussianDropout"]
...
class DropBlock(nn.Module):
    """Randomly drop some regions of feature maps.Please refer to the method proposed in `DropBlock
     <https://arxiv.org/abs/1810.12890>` for details. Copyright (c) OpenMMLab. All rights reserved.
    ...
    Args:
        ...
        warmup_iters (int): The drop probability will linearly increase
            ...
            total_iters = ceil(The total number of samples in the dataset / batch_size) * total_epochs
            warmup_iters = int(total_iters * 0.05)  # 5%~10%
    """
    def __init__(self, drop_prob, block_size, warmup_iters=2000, eps=1e-6):
    ...
    def forward(self, x):
        ...
        mask = 1 - mask
        x = x * mask * mask.numel() / (self.esp + mask.sum())
        return x
