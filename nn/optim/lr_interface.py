...
from pyzjr.nn.optim.adjust_lr import OnceCycleLR, PolyLR, CosineAnnealingLRWithDecay,\
    CosineAnnealingWarmRestartsWithDecay, WarmUpLR, WarmUpWithTrainloader, CosineAnnealingLR,\
...
def get_lr_scheduler(optimizer, scheduler_type='warmup', init_lr=0.001, total_epochs=100,
                     milestones=None, trainloader=None, decay=True, gamma=1, warmup_epochs=5):
    """
    ...
    Args:
        ...
        warmup_epochs (int): The number of epochs in the learning rate warm-up phase, used for the following scheduler types:
            -'gradual_warm': Total number of epochs for progressive preheating (recommended value 5-10)
            -'warm': When no trainloader is provided, it represents the number of epochs preheated in pure epoch format
            The default value is 5, corresponding to the typical warm-up cycle of large datasets such as ImageNet. The small dataset can be set to 3-5
    ...
    if scheduler_type in ['step']:
    ...
    elif scheduler_type in ['gradual_warm']:
        ...
        if trainloader:
            ...
            scheduler = WarmUpLR(optimizer, warmup_epochs)
    else:
