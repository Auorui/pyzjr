...
from pyzjr.utils.check import is_pil, is_numpy, is_positive_int, is_list_or_tuple
from pyzjr.data.utils.data_utils import get_image_shape
...
class ToTensor(object):
    ...
    def __call__(self, image, target):
        if is_pil(image) and is_pil(target):
            image = self.pil_to_tensor(image)
            target = self.pil_to_tensor(target)
        elif is_numpy(image) and is_numpy(target):
            image = self.cv_to_tensor(image)
            target = self.cv_to_tensor(target)
        image = image.half() if self.half else image.float()  # 将uint8转换为fp16/32
        return image, target
...
class RandomHorizontalFlip(object):
    ...
    def pil_hflip(self, image):
        return image.transpose(Images.FLIP_LEFT_RIGHT)

    def cv_hflip(self, image):
    ...
    def __call__(self, image, target):
        if self.random_float >= self.p:
            if is_pil(image) and is_pil(target):
                ...
                target = self.pil_hflip(target)
            elif is_numpy(image) and is_numpy(target):
                image = self.cv_hflip(image)
                target = self.cv_hflip(target)
        return image, target
...
class RandomAdjustBrightness(AdjustBrightness):
    ...
    def __call__(self, image, target):
        if random.random() >= self.pro:
            return super().__call__(image=image, target=target)
        return image, target
...
class RandomRotation():
    ...
    def __call__(self, image, target):
        ...
        if is_pil(image):
            ...
            target = self.pil_rotate(target, angle)
        elif is_numpy(image):
            image = self.cv_rotate(image, angle)
            target = self.cv_rotate(target, angle)
        return image, target
...
class Grayscale():
    ...
    def pil_to_gray(self, img):
        _, _, channels = get_image_shape(img)
        if channels == 3:
            img = img.convert('L')
...
class Centerzoom():
    ...
    def __call__(self, image, target):
        if is_pil(image):
            ...
            target = self.pil_centerzoom(target)
        elif is_numpy(image):
            image = self.cv_centerzoom(image)
            target = self.cv_centerzoom(target)
        return image, target
...
class Resize(object):
    ...
    def __call__(self, image, target):
        if is_pil(image):
            image = self.pil_resize(image)
            target = self.pil_resize(target)
        elif is_numpy(image):
            image = self.cv_resize(image)
            target = self.cv_resize(target)
        return image, target
...
class RandomCrop(object):
    ...
    def pil_random_crop(self, img, left, top):
        right = left + self.w
        bottom = top + self.h
        cropped_img = img.crop((left, top, right, bottom))
        return cropped_img

    def cv_random_crop(self, img, x, y):
        cropped_img = img[y:y+self.h, x:x+self.w]
        return cropped_img

    def __call__(self, image, target):
        width, height, _ = get_image_shape(image)
        ...
        if is_pil(image):
            image = self.pil_random_crop(image, x, y)
            target = self.pil_random_crop(target, x, y)
        elif is_numpy(image):
            image = self.cv_random_crop(image, x, y)
            target = self.cv_random_crop(target, x, y)
        return image, target

class RandomResizeCrop(RandomCrop):
    """类用于随机调整图像大小并裁剪出一个指定大小的区域。"""
    def __init__(self, size, scale_range=(1., 1.2)):
        super(RandomResizeCrop, self).__init__(size=size)
    ...
    def pil_random_resize(self, img):
        ...
        resized_img = img.resize((new_width, new_height), Images.ANTIALIAS)
        return resized_img
    ...
    def __call__(self, image, target):
        ...
        if is_pil(image):
            ...
            target = self.pil_random_resize(target)
            target = self.pil_random_crop(target, x, y)
        if is_numpy(image):
            image = self.cv_random_resize(image)
            image = self.cv_random_crop(image, x, y)

            target = self.cv_random_resize(target)
            target = self.cv_random_crop(target, x, y)
        return image, target
...
class CenterCrop():
    ...
    def __call__(self, image, target):
        if is_pil(image):
            ...
            target = self.pil_center_crop(target)
        elif is_numpy(image):
            image = self.cv_center_crop(image)
            target = self.cv_center_crop(target)
        return image, target
...
class Pad():
    ...
    def __call__(self, image, target):
        if is_pil(image):
            ...
            target = self.pil_pad(target)
        elif is_numpy(image):
            image = self.cv_pad(image)
            target = self.cv_pad(target)
        return image, target
...
class ResizePad():
    ...
    """
    def __init__(self, size, frame=True):
    ...
    def __call__(self, image, target):
        if is_pil(image):
            ...
            target = self.pil_resizepad(target)
        if is_numpy(image):
            image = self.cv_resizepad(image)
            target = self.cv_resizepad(target)
        return image, target
...
class MeanStdNormalize():
    ...
    def __call__(self, image, target):
        return self.mean_std_normalize(image), target
...
if __name__=="__main__":
    ...
    from resources import pyzjr_datas

    image_path = pyzjr_datas('aircraft')
    mask_path = pyzjr_datas('aircraft_mask')


    transforms = Compose([
        # RandomHorizontalFlip(),
        RandomVerticalFlip(),
        AdjustBrightness(1.8),
    ...
    alpha = 0.5
    if is_pil(trans_img):
        fused = Image.blend(trans_img, trans_mask, alpha=alpha)
    else:
        fused = cv2.addWeighted(trans_img, alpha, trans_mask, 1-alpha, 0)
