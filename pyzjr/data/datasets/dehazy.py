...
from pyzjr.core._tensor import hwc2chw
...
class DeHazeDataset(BaseDataset):
    def __init__(
        ...
        self.data_dir = os.path.join(root_dir, 'test' if is_train else 'train')
    ...
    def __getitem__(self, item):
        ...
        if self.mode:
            [source_img, target_img] = augment([source_img, target_img], self.target_shape, self.edge_decay, self.only_h_flip)
        else:
            [source_img, target_img] = align([source_img, target_img], self.target_shape)

        return hwc2chw(source_img), hwc2chw(target_img)


def augment(imglists, size, edge_decay=0., only_h_flip=False):
...
if __name__=="__main__":
    ...
    data_dir = r"D:\PythonProject\MB_TaylorFormer\DehazeFormer\data\Synscapes"
