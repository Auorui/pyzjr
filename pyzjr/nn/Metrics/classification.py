...
class MultiLabelConfusionMatrix(cls_matrix):
    def __init__(self, num_classes):
        super().__init__(num_classes=num_classes)
        self.matrixs = 0
        self.reset()
    ...
    def reset(self):
        self.matrixs = np.zeros((self.num_classes, 2, 2), dtype=np.int64)
    ...
    def update(self, pred, true):
        ...
        fp = np.sum(pred, axis=0) - tp_sum
        fn = np.sum(true, axis=0) - tp_sum
        tp = tp_sum
        tn = true.shape[0] - tp - fp - fn
